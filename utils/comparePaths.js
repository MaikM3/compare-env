...
const comparePaths = (envPaths) => {
  ...
  const relativeEnvPaths = envPaths.map(envPath => path.relative(cwd, envPath))

  const splitenvPaths = relativeEnvPaths.map((envPath) => envPath.split("/").filter(Boolean));

  const longestPathIndex = splitenvPaths.reduce(
    (longestIndex, splitPath, index) =>
      splitPath.length > splitenvPaths[longestIndex].length ? index : longestIndex,
    0
  );
  ...
  longestPath.forEach((_segment, index) => {
    const correspondingSegments = splitenvPaths.map(splitPath => splitPath[index])
    ...
    if (allSegmentsTheSame) {
      ...
      replacedEnvPaths.forEach((envPath, index) => envPath.push(correspondingSegments[index]))
    }
  })
